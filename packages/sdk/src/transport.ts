// Base HTTP transport for the Expirio SDK.
//
// Goals:
...
// - Typed errors: every non-2xx response is decoded into a `ExpirioError`
...
export interface ClientOptions {
  /**
   * Base URL of the Expirio api-edge worker, e.g.
   * `https://api.expirio.app`. Trailing slash is stripped.
   */
  baseUrl: string;
 ...
 * `Expirio` (the client class wired up in `index.ts`).
 */
export class Transport {
  ...
  constructor(options: ClientOptions) {
    ...
    if (typeof fetchImpl !== "function") {
      throw new TypeError(
        "Expirio SDK requires a fetch implementation. Pass `fetch` in options on platforms without a global.",
      );
    }
...
export function generateRequestId(): string {
  ...
  throw new Error(
    "Expirio SDK requires the Web Crypto API (globalThis.crypto). No source of randomness was found.",
  );
}
