...
describe("Transport — success path", () => {
  it("unwraps the data envelope on GET", async () => {
    ...
    const client = new Expirio({ baseUrl: "https://api.test", fetch });
...
describe("Error decoding — typed branches", () => {
  ...
  const cases: Array<[string, number, new (...args: never[]) => ExpirioError]> = [
  ...
  it("forward-compat: unknown error code → base ExpirioError", async () => {
    ...
    try {
      ...
      expect(err).toBeInstanceOf(ExpirioError);
      expect(err).not.toBeInstanceOf(BadRequestError);
      expect((err as ExpirioError).code).toBe("future_quota_exceeded");
    }
  });
