...
Expirio TypeScript SDK — typed, dependency-free, runtime-agnostic client
for the Expirio control plane API.

## Install (workspace)

This package is internal to the `expirio` monorepo. Add it as a
...
```ts
import { Expirio } from "@saas/sdk";

const client = new Expirio({
  baseUrl: "https://api.expirio.app",
  auth: { kind: "bearer", token: process.env["EXPIRIO_TOKEN"]! },
});
...
All non-2xx responses throw a typed subclass of `ExpirioError`. Branch on
...
| `ExpirioError` (base)   | _any_                 | _any_          |

Unknown error codes (forward-compatible — e.g. a future `quota_exceeded`)
decode to the base `ExpirioError` carrying the raw envelope. Non-JSON 5xx
...
A custom `fetch` implementation can be injected via `new Expirio({ fetch })`.
