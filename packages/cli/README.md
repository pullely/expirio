...
`expirio` — first-class TypeScript CLI for the Expirio control
...
This package is internal to the `expirio` monorepo and is not
...
```
expirio login    [--api-url=URL] [--token=BEARER]
expirio logout
expirio whoami
```
...
```
expirio org list
expirio org use <org-id>
expirio org members
expirio project list
```

Writes (Task 0101):

```
expirio org invite <email> [--role=ROLE] [--idempotency-key=KEY] [--org=ORG_ID]
expirio project create <name> [--idempotency-key=KEY]
expirio env create <project-id> <name> [--idempotency-key=KEY]
expirio api-key create <name> [--scope=SCOPE] [--idempotency-key=KEY]
expirio webhook create <url> [--event=EVENT[,EVENT2,...]] [--idempotency-key=KEY]
```

Cross-resource reads (Task 0101):

```
expirio usage summary    [--metric=METRIC] [--from=ISO] [--to=ISO]
expirio billing summary
expirio audit list       [--limit=N] [--cursor=CURSOR] [--category=CAT] [--all]
```
...
context (`expirio org use <org-id>`). Only `org invite` accepts an
...
The shipped V1 is **token-paste**: `expirio login` prompts for a
...
- `FileTokenStore` fallback: `~/.config/expirio/credentials.json`,
  mode **0600**, parent directory mode **0700**.

Active organization context lives at
`~/.config/expirio/config.json` (mode 0644, not a secret). Override
both via `EXPIRIO_CONFIG_DIR` (used by tests).
