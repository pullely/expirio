// CLI error helpers. Translate `ExpirioError` subclasses (from
...
export class MissingOrgContextError extends Error {
  constructor() {
    super("no active organization (run `expirio org use <id>`)");
...
export function formatCliError({ err, mode }: FormatErrorInput): FormattedError {
  ...
  if (err instanceof UnauthenticatedError) {
    return formatSdk(4, err, "token rejected — run `expirio login` to refresh", mode);
  }
  if (err instanceof ExpirioError) {
