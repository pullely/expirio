...
describe("resolveConfigDir", () => {
  it("respects EXPIRIO_CONFIG_DIR override", () => {
    const prev = process.env["EXPIRIO_CONFIG_DIR"];
    process.env["EXPIRIO_CONFIG_DIR"] = "/tmp/cli-override";
    try {
      expect(resolveConfigDir()).toBe("/tmp/cli-override");
    } finally {
      if (prev === undefined) delete process.env["EXPIRIO_CONFIG_DIR"];
      else process.env["EXPIRIO_CONFIG_DIR"] = prev;
    }
  });
});
