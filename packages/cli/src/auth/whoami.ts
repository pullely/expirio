// `expirio whoami` — read token + cached context, validate via SDK.
...
export interface WhoamiInput {
  ...
  /** SDK factory override for tests. Defaults to constructing `Expirio`. */
  readonly sdkFactory?: (baseUrl: string, token: string) => Expirio;
}
