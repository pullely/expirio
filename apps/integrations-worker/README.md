...
Your backend holds one Expirio API key and **zero GitHub credentials**.
...
const expirio = new Expirio({
  baseUrl: process.env.EXPIRIO_API_URL!,
  token: process.env.EXPIRIO_API_KEY!, // service-principal key
});

// Mint a token scoped to exactly the repos + permissions you need.
// TTL ≤ 1h; mint per task and let it expire — never store it.
const { token } = await expirio.integrations.issueGithubToken(ORG_ID, {
