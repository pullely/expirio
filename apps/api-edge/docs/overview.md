...
Part of the expirio runtime: a Cloudflare Worker deployed per environment (`stage`, `prod`; `dev` is verify-only). Not publicly routable — reached only through `api-edge` service bindings.
...
- **web-console-next** — Next.js 15 + opennextjs/cloudflare delivery of the Expirio web console (per-environment, Workers + Static Assets)
