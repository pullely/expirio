...
// deleted; all wire I/O now goes through the typed `Expirio` resource
...
//   - `createClient` — constructs a `Expirio` against a target, wired
...
//     `Promise<T>` + thrown `ExpirioError` into the same shape the
...
import { Expirio, ExpirioError, type ClientOptions } from "@saas/sdk";
import { apiEdgeWorkersDevUrl } from "./app-config";

export type ApiClient = Expirio;
...
export function createClient(target: ApiTarget, token: string | null): Expirio {
  const opts: ClientOptions = { baseUrl: target.url };
  if (token) opts.auth = { kind: "bearer", token };
  return new Expirio(opts);
}

/**
 * Adapt an SDK promise into the `ApiResult<T>` envelope. Typed
 * `ExpirioError` subclasses surface as `{ ok: false }` carrying the
