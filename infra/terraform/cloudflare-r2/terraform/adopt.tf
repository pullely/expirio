...
data "external" "adopt_documents_bucket" {
  program = ["bash", "-c", <<-EOT
    set -euo pipefail
    name="$(jq -r .name)"
    state_file="$(mktemp)"
    ...
    if [ "$code" = "200" ] && jq -e \
        '[.resources[]? | select(.type == "cloudflare_r2_bucket" and .name == "documents")] | length > 0' \
...
import {
  for_each = toset(compact([data.external.adopt_documents_bucket.result.name]))
  to       = cloudflare_r2_bucket.documents
